    ...
    replacements: [
      ...
      },
      {
        oldText:
          'function Ls(t){let{query:e,tables:r,timezone:n}=t;return r.reduce((o,s)=>{let{schemas:a}=o,{columns:d,name:l,schema:u}=s,f=d.sort((c,N)=>c.position-N.position).reduce',
        newText:
          'function __codexParseStudioColumns(t){return Array.isArray(t)?t:typeof t=="string"?JSON.parse(t):[]}function Ls(t){let{query:e,tables:r,timezone:n}=t;return r.reduce((o,s)=>{let{schemas:a}=o,{columns:d,name:l,schema:u}=s,f=__codexParseStudioColumns(d).sort((c,N)=>c.position-N.position).reduce',
        marker: '__codexParseStudioColumns(d)',
      },
    ],
  },
];
...
for (const patch of patches) {
  ...
  let current = fs.readFileSync(patch.file, 'utf8');
  let changed = false;

  for (const replacement of patch.replacements) {
    if (current.includes(replacement.marker)) {
  ...
  if (changed) {
    fs.writeFileSync(patch.file, current);
